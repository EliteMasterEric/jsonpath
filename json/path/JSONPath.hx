...
class JSONPath {
    ...
    static function queryPaths_ChildFilterSelector(filter:Element, targetNode:JSONNode, rootValue:JSONData):Array<JSONNode> {
        ...
        if (filter == null || targetNode.value == null || targetNode.value.isPrimitive()) return [];
...
class JSONPathLexer {
     ...
     */
        ...
        if (char < 0x20) throw formatError_UnexpectedChar(String.fromCharCode(char));
        
        // Exclude surrogate code points
        if (char > 0xD7FF && char < 0xE000) throw formatError_UnexpectedChar(String.fromCharCode(char));

        // Exclude very high Unicode characters
        if (char > 0x10FFFF) throw formatError_UnexpectedChar(String.fromCharCode(char));
        
        // Exclude quotes and backslash
        if (char == ESCAPE) throw formatError_UnexpectedChar(String.fromCharCode(char));
    ...
    //
        ...
        return isLetter(char) || char == UNDERSCORE
            || (char >= 0x80 && char <= 0xD7FF) || (char >= 0xE000 && char <= 0x10FFFF);
    }
