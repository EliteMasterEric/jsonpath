...
class JSONPathParserTest
{
    ...
    public static function testParse():Void
    {
        ...
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.IndexSelector(2)])
        ]));

        var result = new JSONPathParser().parse("$[2, 3]");
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.IndexSelector(2), Element.IndexSelector(3)])
        ]));
        ...
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.ArraySliceSelector(1, 3, null)])
        ]));
        
        var result = new JSONPathParser().parse("$[5:]");
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.ArraySliceSelector(5, null, null)])
        ]));

        var result = new JSONPathParser().parse("$[1:5:2]");
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.ArraySliceSelector(1, 5, 2)])
        ]));

        var result = new JSONPathParser().parse("$[5:1:-2]");
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.ArraySliceSelector(5, 1, -2)])
        ]));

        var result = new JSONPathParser().parse("$[::-1]");
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.ArraySliceSelector(null, null, -1)])
        ]));
        ...
        Test.assertEquals(result, Element.JSONPathQuery([
            ...
            Element.ChildSegment([Element.FilterSelector(
                Element.LogicalOrExpr([
                    Element.LogicalComparisionExpr(
                        Element.FilterQuery(
                            Element.RelativeQuery([
                                Element.ChildSegment([Element.NameSelector('b')])
                            ])
                        ),
                        '==',
            ...
            Element.ChildSegment([Element.FilterSelector(
                Element.LogicalOrExpr([
                    Element.LogicalComparisionExpr(
                        ...
                        '>',
                        Element.PrimitiveLiteralExpr(NumberLiteral(3.5))
                    )
                ])
            )])
        ]));
        ...
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.ChildSegment([Element.NameSelector('a')]),
            Element.ChildSegment([Element.FilterSelector(
                LogicalOrExpr([LogicalTestQueryExpr(
                    Element.FilterQuery(
                        Element.RelativeQuery([
                            Element.ChildSegment([Element.NameSelector('b')])
                        ])
                ...
                Element.FilterSelector(
                    Element.LogicalOrExpr([
                        Element.LogicalComparisionExpr(
                            ...
                            '<',
                            Element.PrimitiveLiteralExpr(IntegerLiteral(3))
                        ),
                    ])
                ),
                Element.FilterSelector(
                ...
                Element.FilterSelector(
                    Element.LogicalOrExpr([
                        Element.LogicalComparisionExpr(
                            ...
                            '<',
                            Element.PrimitiveLiteralExpr(IntegerLiteral(2))
                        ),
                        Element.LogicalComparisionExpr(
                            Element.FilterQuery(
                                Element.RelativeQuery([
                    ...
                    Element.LogicalOrExpr([
                        Element.LogicalAndExpr([
                            Element.LogicalComparisionExpr(
                                ...
                                '>',
                                Element.PrimitiveLiteralExpr(IntegerLiteral(1))
                            ),
                            Element.LogicalComparisionExpr(
                                Element.FilterQuery(
                                    Element.RelativeQuery([])
                                ...
                                '<',
                                Element.PrimitiveLiteralExpr(IntegerLiteral(4))
                            )
                        ])
                    ])
                )
            ...
            Element.ChildSegment([
                Element.FilterSelector(
                    Element.LogicalOrExpr([
                        Element.LogicalTestQueryExpr(Element.FilterQuery(
                            Element.RelativeQuery([
                                Element.ChildSegment([Element.NameSelector('u')])
                            ])
                        )),
                        Element.LogicalTestQueryExpr(Element.FilterQuery(
                            Element.RelativeQuery([
                                Element.ChildSegment([Element.NameSelector('x')])
                            ])
                        ))
                    ])
                )
        ...
        Test.assertEquals(result, Element.JSONPathQuery([
            Element.DescendantSegment([
                Element.NameSelector('j')
            ])
        ]));
